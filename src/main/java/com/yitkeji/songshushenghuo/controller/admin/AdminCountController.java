...
@Api(tags = {"统计管理"})
@RestController
public class AdminCountController extends BaseAdminController{
    ...
    public Result<StatisticsRes> info(CondReq condReq, HttpServletRequest request) throws BaseException{
        ...
        if(condReq.getLocalCond().size() < 1){
            Statistics statistics = StatisticsService.getSumStatisticsCache(null);
        ...
        Statistics statistics = statisticsService.sumStatistics(null, queryBuilder);
        return Result.success(ObjectUtil.convert(statistics, StatisticsRes.class));
    }
